...
// action: string — sent as n8n `action`, bypasses intent classifier
// message: string — sent as freetext, goes through intent classifier
const ACTIONS = [
  {
    key: 'submitClaim',
    action: 'START_CLAIM',
...
export default function QuickActions({ onSelect, onAction }) {
    ...
    <div className="border-t border-gray-100 bg-white px-4 py-2.5">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
        {ACTIONS.map(({ key, action, icon }) => {
          const label = t(`chat.quickActions.${key}`)
          return (
            <button
              key={key}
              type="button"
              onClick={() => action ? onAction(action, null, label) : onSelect(label)}
